...
class Experiment:
    ...
    def __init__(self, learning_rate=50, dim=40, nneg=50, model="poincare",
                 num_iterations=500, batch_size=128, cuda=False, c=1.0):
        self.model = model
    ...
    def train_and_eval(self):
        ...
        if self.model == "poincare":
            model = MuRP(d, self.dim,self.c)
        elif self.model == "normalized":
            model = NMuR(d, self.dim,self.c)
        else:
            model = MuRE(d, self.dim)
        param_names = [name for name, param in model.named_parameters()]
        #opt = RiemannianSGD(model.parameters(), lr=self.learning_rate, param_names=param_names)
        opt = RiemannianAdam(model.parameters(), lr=self.learning_rate, weight_decay=0.)
        if self.cuda:
            model.cuda()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--curvature", type=float, default=1., nargs="?", help="Curvature of Poincare")

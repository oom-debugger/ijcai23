from torch_rgcn.utils import create_adjaceny_matrix_nc, create_adjaceny_matrix_lp, select_w_init, select_b_init, block_diag, split_spo, schlichtkrull_normal_, generate_inverses, generate_self_loops, stack_matrices, sum_sparse
...
class RelationalGraphConvolutionNC(Module):
    ...
    def _calculate_adj(self):
        ...
        if self.weight_decomp is None:
            ...
            self.weights = torch.einsum('rb, bio -> rio', self.comps, self.bases)
        elif self.weight_decomp == 'block':
            self.weights = block_diag(self.blocks)
        else:
        ...
        self.adj = create_adjaceny_matrix_nc(
    ...
    def forward(self, features=None):
        ...
        if self.diag_weight_matrix:
            assert self.weights.size() == (self.num_relations, in_dim)
        else:
            assert self.weights.size() == (self.num_relations, in_dim, out_dim)

        if self.in_features is None:
            # Message passing if no features are given
            output = torch.mm(self.adj, self.weights.view(self.num_relations * in_dim, out_dim))
            ...
            fw = torch.einsum('ij,kj->kij', features, self.weights)
            fw = torch.reshape(fw, (self.num_relations * self.num_nodes, in_dim))
            output = torch.mm(self.adj, fw)
        elif self.vertical_stacking:
            # Message passing if the adjacency matrix is vertically stacked
            af = torch.spmm(self.adj, features)
            af = af.view(self.num_relations, self.num_nodes, in_dim)
            output = torch.einsum('rio, rni -> no', self.weights, af)
        else:
            # Message passing if the adjacency matrix is horizontally stacked
            fw = torch.einsum('ni, rio -> rno', features, self.weights).contiguous()
            output = torch.mm(self.adj, fw.view(self.num_relations * self.num_nodes, out_dim))
...
class RelationalGraphConvolutionLP(Module):
    ...
    def _calculate_adj(self):
        ...
        self.adj = create_adjaceny_matrix_lp(self.triples, self.num_nodes, self.num_relations, self.vertical_stacking, self_loop_keep_prob, device)
    ...
    def forward(self, triples, features=None):
        ...
        if self.in_features is None:
            ...
            output = torch.mm(self.adj, weights.view(self.num_relations * in_dim, out_dim))
        elif self.vertical_stacking:
            # Message passing if the adjacency matrix is vertically stacked
            if self.weight_decomp == 'block':
                weights = block_diag(self.blocks)

...
def get_edge_tag(data):
    edge_dict = {}
    c = 0
    for t in data:
        if (t[0], t[2]) in edge_dict:
            edge_dict[(t[0], t[2])] = edge_dict[(t[0], t[2])] | (1<<t[1])
            c = c + 1
        else:
            edge_dict[(t[0], t[2])] = 0 | (1<<t[1])
            
    new_data = []
    for d in data:
        new_data.append(d.append(edge_dict[(d[0], d[2])]))
    return data
...
#            get_edge_tag(data[0])
    ...
    # Calculate the edge tag. Edge-Tag is an encoding for the number of relations going from edge i to edge j
